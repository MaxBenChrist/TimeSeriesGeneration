...
@function
def MkSg_AR(N=None, params=None, eta=None, *args, **kwargs):
    varargin = MkSg_AR.varargin
    nargin = MkSg_AR.nargin
    ...
    if nargin < 2 or isempty(params):
        params = matlabarray(cat(0.1))

    if nargin < 3 or isempty(eta):
        eta = 100
    ...
    for i in arange(order + 1, N + eta).reshape(-1):
        x[i] = multiply(params, x[i - order:i - 1]) + noise[i]
